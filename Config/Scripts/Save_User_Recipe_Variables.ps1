Param($synchash, $variables, $userName)
...
function New-UserRecipeConfig ($synchash, $variables, $userName) {
    ...
    New-Item $userSettingsRecipePath -Force -ItemType file
    $doc.save($userSettingsRecipePath)
    
    return ,$userSettingsRecipePath
}

New-UserRecipeConfig -synchash $synchash -variables $variables -userName $userName