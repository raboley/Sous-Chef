...
###########################################################################
param([int]$debug,[PSObject[]]$script,$variables,$database)
## create results file
...
try {
    invoke-sqlcmd -InputFile $script.fullname -serverinstance $server -database $database -ErrorVariable myerror -QueryTimeout 600 -variable $variables |Out-null
} catch {
foreach($err in $error){Write-Host $err}
Write-Host $script.Name "Failed to run on" 'master's  -ForegroundColor Red 
